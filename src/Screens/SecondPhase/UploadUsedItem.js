...
const UploadUsedItem = () => {
  ...
  const getCategoryData = async () => {
    try {
      const data = await fetch(
        'https://www.kanpid.com/api/used-item/get-all-category',
      );
  ...
  const uploadItem = async () => {
    ...
    if (!name) {
      ...
      Toaster('Please provide the image');
      return;
    } else if (!setMainCatId) {
      Toaster('Please select the Category');
      return;
    } else if (!setSubMainCatId) {
      Toaster('Please select the SubCategory');
      return;
    } else {
      ...
      axios({
        method: 'post',
        url: 'https://www.kanpid.com/api/used-item/upload-used-item',
        data: formData,
        headers: {
  ...
  const getSubCateory = async id => {
    try {
      const subFilterCategory = await fetch(
        `https://www.kanpid.com/api/used-item/get-all-sub-category/${id}`,
      );
  ...
  const getAttribute = async id => {
    try {
      const attribute = await fetch(
        `https://www.kanpid.com/api/used-item/get-category-attribute-data/${id}`,
      );
