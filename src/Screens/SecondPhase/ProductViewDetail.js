...
const ProductViewDetail = props => {
  ...
  const submitReport = async () => {
    ...
    if (report == '') {
      Toaster('Please write something in report');
      return;
    }
    setLoading(false);
      ...
      <ScrollView>
        ...
        <View
          ...
          <RenderHTML
            contentWidth={width}
            source={{
              html: `${itemObj.product_description}`,
            }}
            tagsStyles={{
              body: {
                width: 300,
          ...
          <Text style={styles.p3Heading}>
            <Text style={{color: '#000', fontSize: 15}}>Price :</Text> Rs
